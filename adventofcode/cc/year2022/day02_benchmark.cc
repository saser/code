...
static void BM_Part1(benchmark::State& state) {
  const auto input = runfiles::cc::runfiles::Read(
      "code/adventofcode/data/year2022/day02.real.in", argv0);
  if (!input.ok()) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(adventofcode::cc::year2022::day02::Part1(*input));
  }
}
BENCHMARK(BM_Part1);
...
static void BM_Part2(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(adventofcode::cc::year2022::day02::Part2(*input));
  }
}
BENCHMARK(BM_Part2);
